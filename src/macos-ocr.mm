...
static bool normalized_clip_name_candidate(NSString *raw, std::string &out, double &score)
{
    ...
    NSArray<NSString *> *blocked = @[
        @"FPS", @"SHUTTER", @"BAT", @"MEDIA", @"REC", @"RAW", @"LOOK", @"CAM", @"SDI", @"HDMI", @"MON", @"WRS", @"LDS",
        @"TC", @"WB", @"EI", @"ND"
    ];
...
std::string recognize_text_rgba_macos(const std::vector<uint8_t> &rgba, uint32_t width, uint32_t height)
{
    ...
    @autoreleasepool {
        ...
        CGImageRef image = CGImageCreate(width, height, 8, 32, width * 4, colorSpace,
                                         kCGImageAlphaPremultipliedLast | kCGBitmapByteOrder32Big, provider, nullptr,
                                                        ...
                                                        uint32_t height)
{
    ...
    @autoreleasepool {
        ...
        for (VNRecognizedTextObservation *observation in request.results) {
            ...
            first = false;
            char buffer[160];
            std::snprintf(buffer, sizeof(buffer), ",\"x\":%.6f,\"y\":%.6f,\"width\":%.6f,\"height\":%.6f", x, y, w, h);
            json += "{\"text\":\"";
...
bool recognize_clip_name_box_rgba_macos(const std::vector<uint8_t> &rgba, uint32_t width, uint32_t height, double &x,
                                        double &y, double &box_width, double &box_height, std::string &text)
{
    if (rgba.empty() || width == 0 || height == 0)
        return false;
